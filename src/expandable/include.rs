...
pub fn expand(parent_path: &str, item: &Yaml, benchmark: &mut Benchmark) {
  ...
  expand_from_filepath(final_path, benchmark, None);
}

pub fn expand_from_filepath(parent_path: &str, benchmark: &mut Benchmark, accessor: Option<&str>) {
  ...
  for item in items {
    if multi_request::is_that_you(item) {
      ...
      include::expand(parent_path, item, benchmark);
